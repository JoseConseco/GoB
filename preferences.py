...
class GoB_Preferences(AddonPreferences):
    ...
    use_scale: EnumProperty(
        ...
        items=[
            ...
            (
                "BUNITS",
                "Blender Units",
                "Changes Scale depending on Blenders Unit Scale ",
            ),
    ...
    flat_icons: BoolProperty(
        name="Use flat icons", description="Use flat icons without color", default=True
    )  # Default: True

    export_uv_flip_x: BoolProperty(
        name="UV Map Flip X", default=False
    )  # Default: False
    ...
    export_modifiers: EnumProperty(
        ...
        items=[
            (
                "APPLY_EXPORT",
                "Export and Apply",
                "Apply Modifiers in Blender and Export them to ZBrush",
            ),
            (
                "ONLY_EXPORT",
                "Only Export",
                "Export Modifiers to ZBrush but do not apply them in Blender",
            ),
    ...
    export_polygroups: EnumProperty(
        ...
        items=[
            ("FACE_SETS", "from Face Sets", "Create Polygroups from Face Sets"),
            ("MATERIALS", "from Materials", "Create Polygroups from Materials"),
            (
                "VERTEX_GROUPS",
                "from Vertex Groups",
                "Create Polygroups from Vertex Groups",
            ),
    ...
    export_mask: EnumProperty(
        name="Mask",
        description="Mask export options",
        items=[
            (
                "SCULPT_MASK",
                "from Sculpt Mode Mask",
                "Create Mask from Sculpt Mode Mask",
            ),
            ("VERTEX_GROUP_MASK", "from Vertex Group", "Create Mask from Vertex Group"),
            ("NONE", "None", "Do not export a Mask"),
        ],
        default="SCULPT_MASK",
    )
    ...
    import_material: EnumProperty(
        ...
        description="Create Material",
        items=[
            ("TEXTURES", "from Textures", "Create Material inputs from Textures"),
    ...
    import_method: EnumProperty(
        ...
        items=[
            (
                "MANUAL",
                "Manual",
                "Manual Mode requires to press the import every time you send a model from zbrush to import it.",
            ),
    ...
    import_mask: BoolProperty(
        name="Mask", description="Import Mask to Vertex Group", default=True
    )  # Default: True

    import_uv: BoolProperty(
        name="UV Map", description="Import Uv Map from ZBrush", default=True
    )  # Default: True
    import_uv_name: StringProperty(
        name="UV Map", description="Set name for the UV Map", default="UVMap"
    )  # Default: UVMap

    import_uv_flip_x: BoolProperty(
        name="UV Map Flip X", default=False
    )  # Default: False
    import_uv_flip_y: BoolProperty(name="UV Map Flip Y", default=True)  # Default: True

    import_subdiv: BoolProperty(
        name="Subdiv", description="Import Subdivision Levels", default=True
    )  # Default: True

    import_polypaint: BoolProperty(
        name="Polypaint", description="Import Polypaint as Vertex Color", default=True
    )  # Default: True
    ...
    import_polygroups: BoolProperty(
        name="Polygroups", description="Import Polygroup data", default=True
    )  # Default: True
    ...
    import_diffuse_suffix: StringProperty(
        name="Base Color", description="Set Suffix for Base Color Map", default="_diff"
    )  # Default: _diff
    ...
    import_normal_suffix: StringProperty(
        name="Normal Map", description="Set Suffix for Normal Map", default="_norm"
    )  # Default: _norm
